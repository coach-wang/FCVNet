...
def featureExtraction(inputSize1, inputSize2):
    ...
    layer4Size = (layer4.get_shape().as_list()[1], layer4.get_shape().as_list()[2])  
...
def xNet(inputSize1, inputSize2, learningRate):
    ...
    '''features'''
    featureExtractionLayer = featureExtraction(inputSize1, inputSize2)  
    featureList1 = []
    featureList2 = []
    featureList3 = []
    featureList4 = []
    ...
    model = Model(inputs=inputList, outputs=[pred, dirAtt]) # testing attention
    #model = Model(inputs=inputList, outputs=[pred]) # test
